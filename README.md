...
**M3: アプリ検出 + TCC 権限スケルトン 完了 (2026-04-23)** — SoundManagerDriver が `OnAddClient` + `StartIOImpl/StopIOImpl` で client-specific に I/O 状態を追跡、カスタムプロパティ `kSMCustomPropertyActiveClients` で "実際に音を書き込んでいる client" だけを UI に公開。UI 側は bridging header 経由で購読し、`ProcessGrouping` が helper プロセス統合 (Chrome の複数 helper → Google Chrome 1 件に集約) とシステムサービス除外を行い、`RunningAppsMonitor` が NSWorkspace からアイコンと表示名を解決。Menu bar に「再生中のアプリ」セクション (アイコン + 表示名 + 音量スライダ 2 段構成) とセットアップ Window を追加。テスト 28 件全 pass。
...
| M3: アプリ検出 + TCC 権限フロー | 完了 (2026-04-23) |
| M4: 個別音量調整 + エッジケース対応 | 次に着手 |
